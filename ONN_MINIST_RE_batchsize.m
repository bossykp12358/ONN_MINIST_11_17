...
batch_num=[128,256,512,1024,2048,4096];

iteration_num=[floor(n_train/128),floor(n_train/256),floor(n_train/512),floor(n_train/1024),floor(n_train/2048),floor(n_train/4096)];

epoch_num=floor(3000./iteration_num);
...
batch_train=batch_num(model_num);%每一批的训练数

iteration=iteration_num(model_num);
...
co=rand(1,3);
legend_str{model_num}=['小批量训练每批的尺寸',num2str(batch_train)];
