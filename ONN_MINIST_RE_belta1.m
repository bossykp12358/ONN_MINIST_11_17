...
belta_1_list=[0.9,0.99,0.999,0.9999];

belta_2_list=[0.9,0.99,0.999,0.9999,0.99999,0.999999];
...
%采用Adam梯度下降优化法
belta_1=belta_1_list(model_num);%移动指数加权平均系数
belta_2=0.999;%方均根系数
...
for iter = 1 : 1 : iteration    
    ...
    for i=2:num_hidden_layers
        ...
        % vdW{i}=vdW{i}/(1-belta_1^((p-1)*(iteration)+iter));       %修正一阶矩
        % 
        % sdW{i}=sdW{i}/(1-belta_2^((p-1)*(iteration)+iter));       %修正二阶矩
        % 
        % weights{i} = weights{i} - lr * dCdW{i}-landa*lr*(weights{i});
...
co=rand(1,3);
legend_str{model_num}=['Adam优化器belta1参数',num2str(belta_1)];
