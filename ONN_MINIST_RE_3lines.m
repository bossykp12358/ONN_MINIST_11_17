...
%设置一系列参数和超参数
num_hidden_layers = 3;%隐层数量

hidden_layer_size = 49;%隐层神经元个数

learning_rate = 1e-10;%学习率
...
batch_train=600;%每一批的训练数

batch_test=100;%每一批的训练数

iteration=n_train/batch_train;%迭代的次数
...
for i=2:num_hidden_layers
    ...
    weights{i}=rand(hidden_layer_size, hidden_layer_size);
...
for iter = 1 : 1 : iteration    
    ...
    %计算损失
    cost_train(iteration*(p-1)+iter)=mse(Y_train(:,1+batch_train*(iter-1):batch_train*iter),A{i+1});
    %cost_train(iter)=mse(Y_train,A{i+1});
    
    %开始反向传播误差
    dCdA=(A{i+1}-Y_train(:,1+batch_train*(iter-1):batch_train*iter)); %求导链式，损失函数对输出的求导
    %dCdA=(A{i+1}-Y_train);
    ...
    dCdWfinal=dCdA.*dAdB*dBdW; %损失函数对最后一层连接层的求导
    
    dCdW=cell(num_hidden_layers+1, 1); %创建结构体，损失函数对权重的导数结构体
   
    dCdW{num_hidden_layers+1}=dCdWfinal;
    
    dCda=weights{num_hidden_layers+1}'*(dCdA.*dAdB);%损失函数对倒数第二层的求导，这个变量是更新的，随着循环的进行一直往前走
    ...
    %误差传递到第一层
    %dCdW{1}=dCda.*relu_derivative(B{1})*A{1}';
    dCdW{1}=0;
    
    %权重进行更新，
    for i=1:num_hidden_layers+1
        
        weights{i} = weights{i} - learning_rate *(dCdW{i}+landa*sgn(weights{i}));
        
    end
    

    %% 测试集测试
    B_T = X_test(:,1+batch_test*(iter-1):batch_test*iter);
    ...
    y_predict = relu(T_final);

    cost_test(iteration*(p-1)+iter)=mse(Y_test(:,1+batch_test*(iter-1):batch_test*iter),y_predict);
    ...
    %     if y_predict(i)<0.5
    %         y_predict(i)=0;
    %     elseif (0.5<=y_predict(i))&&(y_predict(i)<1.5)
    %         y_predict(i)=1;
    %     elseif (1.5<=y_predict(i))&&(y_predict(i)<2.5)
    %         y_predict(i)=2;
    %     elseif (2.5<=y_predict(i))&&(y_predict(i)<3.5)
    %         y_predict(i)=3;
    %     elseif (3.5<=y_predict(i))&&(y_predict(i)<4.5)
    %         y_predict(i)=4;
    %     elseif (4.5<=y_predict(i))&&(y_predict(i)<5.5)
    %         y_predict(i)=5;
    %     elseif (5.5<=y_predict(i))&&(y_predict(i)<6.5)
    %         y_predict(i)=6;
    %     elseif (6.5<=y_predict(i))&&(y_predict(i)<7.5)
    %         y_predict(i)=7;
    %     elseif (7.5<=y_predict(i))&&(y_predict(i)<8.5)
    %         y_predict(i)=8;
    %     elseif (8.5<=y_predict(i))
    %         y_predict(i)=9;
    %     end
    % 
    ...
    for i=1:1:size(y_predict,2)
        ...
        % for j=1:1:size(y_predict,1)
        % 
        %     if j == find(y_predict(:,i)==max(y_predict(:,i)))
        %         y_predict(j,i)=1;
        %     else 
        %         y_predict(j,i)=0;
        %     end
