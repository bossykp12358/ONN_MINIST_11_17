...
%weights_final_init=weights{num_hidden_layers+1};
...
for iter = 1 : 1 : iteration    
    ...
    dCdWfinal=dCdB*dBdW/batch_train;
    ...
    for i=num_hidden_layers:-1:2
        ...
        for j=1:hidden_layer_size        
            if j== 1            
                inds=j:j+1; %下标 
                dCdW{i}(j, j)=dCda(j,:).*relu_derivative(B{i}(j))*A{i}(j,:)'/batch_train;%对于权重的导数
...
%legend_str{model_num}=['有三个支路的拓扑结构'];
